...
//init_sensors();
  ...
  peakDetection2.begin(48, 3, 0.6);               // sets the lag,F threshold and influence


  peakDetection3.begin(48, 3, 0.6);               // sets the lag,F threshold and influence
...
unsigned long cooldownPeriod2 = 800; // Cooldown period in milliseconds (adjust as needed)



float prev_velocity_x = 0;
float prev_velocity_y = 0;
float prev_velocity_z = 0;

unsigned long prev_time_acc = 0;
unsigned long current_time_acc = 0;
...
//        current_time_acc = millis();
//
//        float dt = (current_time_acc - prev_time_acc) / 1000.0; // Convert milliseconds to seconds
//
//        prev_time_acc = current_time_acc;
//
//        float velocity_x = prev_velocity_x + ax_x  * dt;
//        float velocity_y = prev_velocity_y + ax_y  * dt;
//        float velocity_z = prev_velocity_z + ax_z  * dt;
//        float acc = sqrt(abs(ax_x+ax_y+ax_z));
//        peakDetection2.add(ax_z);
//        double filteredacc = peakDetection2.getFilt();
//        Serial.println(filteredacc); // End line
 
//        Serial.print(ax_x); // Print heart rate
//        Serial.print(","); // Delimiter
//        Serial.print(ax_y); // Print SpO2
//        Serial.print(","); // Delimiter
//        Serial.print(ax_z); // Print SpO2
//        Serial.println(); // End line
  ...
  if (abs(-frequency_0 - lastValue) <= 10) {
...
//          Serial.println(filtered+10);                    // print moving average
...
//          Serial.println(filtered-10);                    // print moving average
    ...
    //        mqttClient.publish("iot", "Minima");
        }else if (sum2 < 1 && sum1 > 1 && (millis() - lastPeakTime2) > cooldownPeriod2) {
//          Serial.println(filtered+10);                    // print moving average
