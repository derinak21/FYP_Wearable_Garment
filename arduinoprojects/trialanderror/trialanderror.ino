...
//    Serial.println(count);                    // print moving average
    ...
    int lastcurrent = currentIndex- 5;
    if (lastcurrent < 0){
...
//    Serial.println("holding breath");
      ...
      snprintf(message, sizeof(message), "Holding");
...
//  Serial.println(-frequency_0);
  if ((&averagedFrequencyReadings[lastcurrent] == maxElement) && (difference > 4)) {
...
//       Serial.println(filtered); 
      ...
      snprintf(message, sizeof(message), "Peak, Peak_amplitude: %.2f Respiratory_rate: %.2f", amp, respiratoryRate);
